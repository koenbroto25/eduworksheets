DROP FUNCTION IF EXISTS get_student_class_assignments(uuid);
...
RETURNS TABLE(
    ...
    status progress_status_enum, -- Changed from text to progress_status_enum
    score int
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    SELECT
        ...
        ce.assigned_at,
        ce.due_date,
        COALESCE(up.status, 'not_started'::progress_status_enum) AS status, -- Explicitly cast to the enum type
