-- Drop the previous version of the function to apply the fix.
DROP FUNCTION IF EXISTS get_student_class_report(UUID, UUID);

-- Recreate the function, ensuring assignments defaults to an empty array instead of NULL.
...
BEGIN
    ...
    SELECT
        ...
        (SELECT COUNT(*) FROM student_progress WHERE status::text LIKE 'completed%') as total_assignments_completed,
        ...
        -- FIX: Coalesce the result of jsonb_agg to an empty JSON array '[]' if it's NULL.
        -- This prevents the frontend from crashing when a class has no assignments.
        COALESCE(
            (
                SELECT jsonb_agg(
                    jsonb_build_object(
...
COMMENT ON FUNCTION get_student_class_report(UUID, UUID) IS 'Generates a detailed report of a student''s performance in a specific class. v3 - Fixes null assignments array.';
