...
DROP POLICY IF EXISTS "Users can create comments" ON exercise_comments;
CREATE POLICY "Users can create comments"
ON exercise_comments 
    FOR INSERT TO authenticated 
    WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can read comments" ON exercise_comments;
...
DROP POLICY IF EXISTS "Teachers can manage announcements for their classes" ON class_announcements;
...
DROP POLICY IF EXISTS "Students can view announcements for their classes" ON class_announcements;
...
DROP POLICY IF EXISTS "Parents can manage their own assignments" ON parent_assignments;
CREATE POLICY "Parents can manage their own assignments"
ON parent_assignments
FOR ALL
USING (auth.uid() = parent_id);

DROP POLICY IF EXISTS "Children can view assignments given to them" ON parent_assignments;
CREATE POLICY "Children can view assignments given to them"
ON parent_assignments
FOR SELECT
USING (auth.uid() = child_id);

DROP POLICY IF EXISTS "Enable read access for authenticated users" ON public.parent_assignments;
...
DROP POLICY IF EXISTS "Parents can create invitations" ON parent_invitations;
CREATE POLICY "Parents can create invitations"
ON parent_invitations FOR INSERT
TO authenticated
WITH CHECK (auth.uid() = parent_id);

DROP POLICY IF EXISTS "Parents can see their own invitations" ON public.parent_invitations;
...
DROP POLICY IF EXISTS "Users can see specific invitations" ON public.parent_invitations;
...
DROP POLICY IF EXISTS "Parents can update their invitations" ON public.parent_invitations;
...
DROP POLICY IF EXISTS "Children can accept invitations" ON public.parent_invitations;
